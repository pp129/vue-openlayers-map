  ...
  if (!me._ismove) {
    ...
    me._intervalTraceFlag = setInterval(function () {
      ...
      if (
        me._fromPause ||
        me._fromStop ||
        me._pointIndex >= tracePoints.length - 1 ||
        me._pointIndex >= tracePointsFromTime.length
      ) {
        return;
      }
...
LushuTrack.prototype.getTracePoints = function (step) {
  ...
  for (let n = 0; n < _pathInfo.length - 1; n++) {
    ...
    for (let p = 0; p < pointsSum + 1; p++) {
      ...
      breakPoints.push({ rotate: angle, coordinate, thisTimeSecond });
      temBreakPointsFromTime.push({ rotate: angle, coordinate, thisTimeSecond });
    }
    if (length % step !== 0) {
    ...
    breakPointsFromTime.push({
      ...
      initTime,
      targetTime,
      temBreakPointsFromTime,
    });
  }
  return { breakPoints, breakPointsFromTime };
};
...
function formatDateTime(theDate) {
  ...
  const _year = theDate.getFullYear();
...
function drawText(map, pointsArr, zoom, textBounds, textLayer) {
  ...
  for (let p = 0; p < pointsArr.length; p++) {
    const element = pointsArr[p];
    const pixel = map.getPixelFromCoordinate(element.position);
    if (!containsCoordinate(map.getView().calculateExtent(), element.position)) {
      continue;
    }
    const a = /[\u4e00-\u9fa5]/g;
    const textLen = element.text.match(a) ? element.text.match(a).length : 0;
    const numberLen = element.text.length - textLen;
    const textPixel = textLen * 15 + numberLen * 8;

    const rightTopMinx = pixel[0] + 10;
    const rightTopMaxx = pixel[0] + textPixel + 20;
    const rightTopMiny = pixel[1] - 32;
    const rightTopMaxy = pixel[1] - 12;
    const rightTopCenter = [rightTopMinx, parseInt((rightTopMiny + rightTopMaxy) / 2)];
    const rightTopMin = { x: rightTopMinx, y: rightTopMiny };
    const rightTopMax = { x: rightTopMaxx, y: rightTopMaxy };

    const leftTopMinx = pixel[0] - textPixel - 20;
    const leftTopMaxx = pixel[0] - 10;
    const leftTopMiny = pixel[1] - 32;
    const leftTopMaxy = pixel[1] - 12;
    const leftTopCenter = [leftTopMaxx, parseInt((leftTopMiny + leftTopMaxy) / 2)];
    const leftTopMin = { x: leftTopMinx, y: leftTopMiny };
    const leftTopMax = { x: leftTopMaxx, y: leftTopMaxy };

    const leftBomMinx = pixel[0] - textPixel - 20;
    const leftBomMaxx = pixel[0] - 10;
    const leftBomMiny = pixel[1] + 12;
    const leftBomMaxy = pixel[1] + 32;
    const leftBomCenter = [leftBomMaxx, parseInt((leftBomMiny + leftBomMaxy) / 2)];
    const leftBomMin = { x: leftBomMinx, y: leftBomMiny };
    const leftBomMax = { x: leftBomMaxx, y: leftBomMaxy };

    const rightBomMinx = pixel[0] + 10;
    const rightBomMaxx = pixel[0] + textPixel + 20;
    const rightBomMiny = pixel[1] + 12;
    const rightBomMaxy = pixel[1] + 32;
    const rightBomCenter = [rightBomMinx, parseInt((rightBomMiny + rightBomMaxy) / 2)];
    const rightBomMin = { x: rightBomMinx, y: rightBomMiny };
    const rightBomMax = { x: rightBomMaxx, y: rightBomMaxy };
    ...
    for (let r = 0; r < 4; r++) {
      for (let i = 0; i < textBounds.length; i++) {
        const pointBounds = textBounds[i];
        if (r === 0) {
    ...
    const leftTop = map.getCoordinateFromPixel([minx, miny]);
    const rightTop = map.getCoordinateFromPixel([maxx, miny]);
    const rightBom = map.getCoordinateFromPixel([maxx, maxy]);
    const leftBom = map.getCoordinateFromPixel([minx, maxy]);
...
export const PathSimplifier = function (options) {
  let { map, paths, id, opts, vacuate, smokeMode, tracePointsModePlay, vacuateDistance, labelShow, labelStyle, showTracePoint } =
    options;
  opts.id = id || nanoid();
