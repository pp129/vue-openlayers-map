...
export default class vzPath {
  ...
  constructor(options) {
    ...
    // 车辆动画图标
    const imageLoad = new Icon(this._operators.carIcon);
    imageLoad.load();
  ...
  simplifyOpera(eve) {
    ...
    if (
      ...
      this._animateLine
        .getGeometry()
        ?.setCoordinates(
          reShape.slice(0, reNode.indexOf(this._moveIdx) + 1).concat([this._animateStart.getGeometry().getCoordinates()])
        );
    }
  ...
  eventListener(mbEvent) {
    ...
    const hasFlag = this._map?.hasFeatureAtPixel(mbEvent.pixel, {
      layerFilter: function (layer) {
        return (
          layer instanceof VectorLayer && (layer.get("track-type") === "trackPath" || layer.get("track-type") === "trackNodes")
        );
      },
      hitTolerance: 0,
    });
    if (hasFlag) {
      ...
      this._moveNodes =
        this._map?.getFeaturesAtPixel(mbEvent.pixel, {
          layerFilter: function (layer) {
            return layer instanceof VectorLayer && layer.get("track-type") === "trackNodes";
          },
          hitTolerance: 0,
        }) || [];
      ...
      this._movePath =
        this._map?.getFeaturesAtPixel(mbEvent.pixel, {
          layerFilter: function (layer) {
            return layer instanceof VectorLayer && layer.get("track-type") === "trackPath";
          },
          hitTolerance: 0,
        }) || [];
  ...
  initLinesAndMarkers() {
    ...
    for (const poi of path) {
      ...
      if (nodeIndex === 0) {
        ...
        start.setStyle(
          new Style({
            image: new Icon(this._operators.startIcon),
            text: new Text({
              text: "",
            }),
          })
        ...
        const end = node.clone();
          ...
          new Style({
            image: new Icon(this._operators.endIcon),
            text: new Text({
              text: "",
            }),
          })
  ...
  stop(ended) {
    ...
    this._originPath instanceof LineString &&
      this._animateStart.getGeometry()?.setCoordinates(this._originPath.getFirstCoordinate());
  }
